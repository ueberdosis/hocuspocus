...
export class ClientConnection<Context = any> {
  ...
  constructor(
    ...
    this.maxUnauthenticatedQueueSize = opts.maxUnauthenticatedQueueSize ?? 5 * 1024 * 1024
    this.maxUnauthenticatedQueueMessages = opts.maxUnauthenticatedQueueMessages ?? 1_000
  ...
  private setUpNewConnection = async (
    rawKey: string,
    documentName: string,
    sessionId: string | null,
  ) => {
  ...
  private handleQueueingMessage = async (
    data: Uint8Array,
    rawKey: string,
    documentName: string,
  ) => {
    try {
      ...
      if (!(type === MessageType.Auth && !this.documentConnectionsEstablished.has(rawKey))) {
