...
export class Document extends Doc {
  ...
  private handleAwarenessUpdate(
    ...
    const originConnection: Connection | null =
      isTransactionOrigin(origin) && origin.source === 'connection' ? origin.connection : null
  ...
  private broadcastUpdate(update: Uint8Array): void {
    this.broadcast(address =>
      new OutgoingMessage(address).createSyncMessage().writeUpdate(update).toUint8Array(),
    )
  }
   ...
   */
  public relayStateless(payload: string, filter?: (conn: Connection) => boolean): void {
    this.broadcast(
      address => new OutgoingMessage(address).writeStateless(payload).toUint8Array(),
      filter,
    )
  }
