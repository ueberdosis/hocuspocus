...
export interface ServerConfiguration<Context = any> extends Configuration<Context> {
  port?: number
  address?: string
...
export class Server<Context = any> {
  ...
  async listen(port?: number, callback: any = null): Promise<Hocuspocus<Context>> {
    if (port) {
    ...
    return new Promise((resolve: Function, reject: Function) => {
      ...
      this.httpServer.listen(listenOptions, async () => {
