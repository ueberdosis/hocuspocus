...
import { MessageType, type ConnectionTransactionOrigin, type TransactionOrigin } from './types.ts'
  ...
  async readSyncMessage(
    ...
    switch (type) {
      ...
      case messageYjsSyncStep2: {
        ...
        if (connection) {
          connection.send(new OutgoingMessage(messageAddress).writeSyncStatus(true).toUint8Array())
        }
        break
      }
