...
import { ExecutionError, type ExecutionResult, type Lock, Redlock } from '@sesamecare-oss/redlock'
...
export class Redis implements Extension {
   ...
   */
  private async syncInitialStateFromPeers(documentName: string, document: Document) {
  ...
  private async hasOtherSubscribers(documentName: string): Promise<boolean> {
    try {
      const result = (await this.pub.pubsub('NUMSUB', this.subKey(documentName))) as [
        string,
        number | string,
      ]
   ...
   * its own fan-out.
   */
  private schedulePublish(documentName: string, document: Document): PendingPublish {
    ...
    const handle = setImmediate(() => {
      this.flushPublish(documentName).catch(error => {
        console.error(`Redis: failed to publish for document "${documentName}"`, error)
      })
    })
  ...
  private async flushPublish(documentName: string) {
    ...
    if (
      ...
      publishes.push(
        this.pub.publish(this.pubKey(documentName), this.encodeMessage(message.toUint8Array())),
      )
    }
  ...
  async onStoreDocument({ documentName }: onStoreDocumentPayload) {
    ...
    try {
      ...
      if (
        ...
        error.message === 'The operation was unable to achieve a quorum during its retry window.'
      ) {
   ...
   */
  async afterStoreDocument({ documentName, lastTransactionOrigin }: afterStoreDocumentPayload) {
  ...
  public async onChange(data: onChangePayload): Promise<any> {
    ...
    this.schedulePublish(data.documentName, data.document).syncStepPending = true
  }
