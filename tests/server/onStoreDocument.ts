...
describe('onStoreDocument', () => {
      ...
      const provider = newHocuspocusProvider(hocuspocus, {
        onSynced() {
          ...
          setTimeout(async () => {
            ...
            expect(executedOnChange).toBe(5)
            expect(executedOnStoreDocument).toBe(1)
  ...
  test('stops when one of the onStoreDocument hooks throws a non-Error value', async t => {
    await new Promise(async resolve => {
      ...
      class NotExecuted {
        async onStoreDocument() {
          // This MUST NOT be executed.
          expect.fail('NotExecuted should not run')
        }
      }
    ...
    await new Promise(async resolve => {
      ...
      const provider = newHocuspocusProvider(server, {
        onSynced() {
          ...
          setTimeout(() => {
            expect(triggered).toStrictEqual(['one', 'two'])
  ...
  test('allows to overwrite the order of extension with a priority', async t => {
    await new Promise(async resolve => {
      ...
      const server = await newHocuspocus({
        ...
        extensions: [new Running(), new BreakTheChain(), new NotRunning(), new HighPriority()],
      })
      ...
      const provider = newHocuspocusProvider(server, {
        onSynced() {
          ...
          setTimeout(() => {
            expect(triggered).toStrictEqual(['zero', 'one', 'two'])
    ...
    await new Promise(async resolve => {
      ...
      const server = await newHocuspocus({
        ...
        async onStoreDocument() {
          ...
          if (startTime === 0) {
            expect.fail('startTime not set')
          } else if (endTime - startTime < 500) {
            expect.fail('did not wait 500ms to call onStoreDocument when closing')
          } else {
            pass()
          }
          resolve('done')
      ...
      const server = await newHocuspocus({
        ...
        async onStoreDocument() {
          ...
          if (startTime === 0) {
            ...
            expect.fail('did not call onStoreDocument immediately when closing')
          }
          resolve('done')
        },
      })
  ...
  test('does not call the onStoreDocument hook if document is not changed after the last user disconnects', async t => {
    await new Promise(async resolve => {
      const server = await newHocuspocus({
        async onStoreDocument() {
          expect.fail('no need to call onStoreDocument if the document is not changed')
        },
      })
        ...
        extensions: [
          {
            ...
            async afterUnloadDocument(data) {
              expect(saveFinished, true).toStrictEqual(
                'Unload should occur only after save was finished',
              )
              pass()
              resolve('done')
            },
        ...
        const provider2 = newHocuspocusProvider(server, {
          ...
          async onSynced() {
            setTimeout(() => {
              ...
              expect(value, ['foo', 'bar']).toStrictEqual('Client 2 should see both changes')
              pass()
              resolve('done')
            }, 500)
          },
      ...
      setTimeout(() => {
        ...
        const provider2 = newHocuspocusProvider(server, {
          ...
          async onSynced() {
            setTimeout(() => {
              expect(
                getCurrentValue(provider2.document),
                'Client 2 should see both changes',
