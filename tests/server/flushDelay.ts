...
describe('flushDelay', () => {
  ...
  test('batches by default', async t => {
    ...
    expect(document.flushDelay).toBe(0)
  ...
  test('a server batches by default', async t => {
    ...
    expect(server.configuration.flushDelay).toBe(0)
  })
  ...
  test('collapses awareness changes to the latest state of each client', async t => {
    ...
    expect(states.get(document.clientID).cursor).toBe(3)
  })
  ...
  test('destroy flushes what is still buffered instead of dropping it', async t => {
    ...
    expect(batchedConnection.sent.length).toBe(0)
    ...
    expect(batchedConnection.sent.length).toBe(afterDestroy)
    ...
    expect(batchedConnection.sent.length).toBe(unbatchedConnection.sent.length)
  })

  test('a batching server still converges on connected clients', async t => {
    const server = await newHocuspocus({ flushDelay: 0 })

    const writer = newHocuspocusProvider(server, { name: 'flush-converge' })
    const reader = newHocuspocusProvider(server, { name: 'flush-converge' })
