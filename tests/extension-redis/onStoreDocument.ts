    ...
    await new Promise(async resolve => {
      ...
      class CustomStorageExtension {
        async onStoreDocument({ document }: onStoreDocumentPayload) {
          expect(document.getArray('foo').get(0)).toBe('bar')
          expect(document.getArray('foo').get(0)).toBe(
    ...
    await new Promise(async resolve => {
      ...
      const server = await newHocuspocus({
        ...
        onStoreDocument: async ({ document }) => {
          expect(provider.document.getArray('foo').get(0)).toBe(document.getArray('foo').get(0))
      ...
      const anotherProvider = newHocuspocusProvider(anotherServer, {
        onSynced() {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              expect(anotherServer.documents.size).toBe(0)
